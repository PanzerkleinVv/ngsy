...
public class DutiesUnitController {
	...
	@RequestMapping("/save")
	@RequiresRoles(value = RoleSign.ADMIN)
	public String save(String unitId, DutiesUnitList dutieUnits, RedirectAttributes model, HttpSession session)
			throws Exception {
		if (unitId != null && !"".equals(unitId)) {
			if (dutieUnits.getDutieUnits() != null && dutieUnits.getDutieUnits().size() > 0) {
				final ResultMessage message = new ResultMessage();
				final Long userId = (Long) session.getAttribute("userId");
				final String METHOD = "更新行政职务：";
				message.edit();
				if (dutiesUnitService.modify(dutieUnits.getDutieUnits(), unitId)) {
					message.success();
	...
	@RequiresRoles(value = RoleSign.ADMIN)
	public Boolean checkUsed(DutiesUnit dutiesUnit) {
		long count = dutiesPersonService.countXianrenByDutiesId(dutiesUnit.getId());
		if (count > 0) {
			return true;
		} else {
			return false;
