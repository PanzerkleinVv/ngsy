...
jQuery.validator.addMethod("checkIdCard", function(value, element) {
	var card = value;
	if (card === '') {
		$(element).data('请输入身份证号，身份证号不能为空');
		return false;
	}
	card = card.toUpperCase();
	// 校验长度，类型
	if (isCardNo(card) === false) {
		$(element).data('您输入的身份证号码不正确，请重新输入');
		return false;
	}
	// 检查省份
	...
	if (checkBirthday(card) === false) {
		$(element).data('您输入的身份证号码生日不正确,请重新输入');
		return false;
	}
	// 检验位的检测
	if (checkParity(card) === false) {
		$(element).data('您的身份证校验位不正确,请重新输入');
		return false;
	}
	// 检验是否已用
	if (checkUsed(card) == false) {
		$(element).data('您输入的身份证号码已被使用,请重新输入');
		return false;
	}
	return true;
}, "请正确输入身份证号码");
...
checkUsed = function(card) {
	...
	$.ajax({
		type : "GET",
		url : "rest/person/checkIdCard.jsp?idCard=" + card + "&id=" + id,
		dataType : "text",
		async : false,
